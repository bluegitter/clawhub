...
type LocalSkillDiffCardProps = {
  slug: string;
  versions: LocalSkillDetailData["versions"];
...
export function LocalSkillDiffCard({ slug, versions, tags }: LocalSkillDiffCardProps) {
  ...
  const versionCacheRef = useRef(new Map<string, Awaited<ReturnType<typeof fetchLocalVersionDetail>>>());
  const textCacheRef = useRef(new Map<string, string>());
  const userSelectedViewModeRef = useRef(false);

  const versionEntries = useMemo(
    () => versions.map((entry) => ({ id: entry.version, version: entry.version })),
    [versions],
  );
  const orderedVersions = useMemo(() => sortVersionsBySemver(versionEntries), [versionEntries]);
  const versionById = useMemo(() => new Map(versions.map((entry) => [entry.version, entry])), [versions]);

  const latestId = useMemo(() => resolveLatestVersionId(versionEntries, tags), [tags, versionEntries]);
  const previousId = useMemo(
    () => resolvePreviousVersionId(versionEntries, latestId),
    [latestId, versionEntries],
  );
  ...
  const versionOptions = useMemo(() => {
    ...
    return options;
  }, [latestId, orderedVersions, previousId, tags, versionById, versions]);

  useEffect(() => {
    if (!versions.length) return;
    const defaults = getDefaultDiffSelection(versionEntries, tags);
    setLeftVersionId((current) => (current && versionById.has(current) ? current : defaults.leftId));
    setRightVersionId((current) => (current && versionById.has(current) ? current : defaults.rightId));
  }, [tags, versionById, versionEntries, versions.length]);
  ...
  const [leftFiles, setLeftFiles] = useState<FileMeta[]>([]);
  const [rightFiles, setRightFiles] = useState<FileMeta[]>([]);

  useEffect(() => {
    let cancelled = false;

    async function loadVersionFiles(versionId: string | null) {
      if (!versionId) return [];
      ...
      const detail = await fetchLocalVersionDetail(slug, versionId);
      versionCacheRef.current.set(versionId, detail);
    ...
    void Promise.all([loadVersionFiles(leftVersionId), loadVersionFiles(rightVersionId)])
      .then(([nextLeft, nextRight]) => {
        if (cancelled) return;
        setLeftFiles(nextLeft);
        setRightFiles(nextRight);
      })
      .catch((loadError) => {
        if (cancelled) return;
        setError(loadError instanceof Error ? loadError.message : "Failed to load version files.");
        setLeftFiles([]);
        setRightFiles([]);
      });
  ...
  }, [leftVersionId, rightVersionId, slug]);

  const fileDiffItems = useMemo(() => buildFileDiffList(leftFiles, rightFiles), [leftFiles, rightFiles]);
  ...
  useEffect(() => {
    ...
    async function loadText(versionId: string, path: string) {
      ...
      const text = await fetchLocalSkillFile(slug, versionId, path);
  ...
  }, [leftVersionId, rightVersionId, selectedItem, slug]);
  ...
  return (
    <div className="diff-card diff-card-embedded">
      ...
      <div className="diff-controls">
        <div className="diff-select">
          <label htmlFor="local-diff-left">Left</label>
          <select
            id="local-diff-left"
            className="search-input"
            value={leftVersionId ?? ""}
            onChange={(event) => setLeftVersionId(event.target.value || null)}
          >
        ...
        <div className="diff-select">
          <label htmlFor="local-diff-right">Right</label>
          <select
            id="local-diff-right"
            className="search-input"
            value={rightVersionId ?? ""}
            onChange={(event) => setRightVersionId(event.target.value || null)}
          >
            <option value="" disabled>
              Select version
            </option>
        ...
        <div className="diff-view">
          {error ? (
            ...
            <div className="diff-empty">
              {sizeWarning.side === "left" ? "Left" : "Right"} file exceeds 200KB: {sizeWarning.path}
            </div>
          ) : diffUnavailable ? (
...
function renderOptions(options: VersionOption[]) {
  ...
  for (const option of options) groups[option.group].push(option);
